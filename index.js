...
function getRandomFlavors(flav1, flav2, flav3, flav4){
  /* let newArray = []
  let randomFlavors = []
  for (i = 0; i < flav1.length; i++){
    newArray.push(flav1[i])
  }
  for (i = 0; i < flav2.length; i++){
    newArray.push(flav2[i])
  }
  for (i = 0; i < flav3.length; i++){
    newArray.push(flav3[i])
  }
  for (i = 0; i < flav4.length; i++){
    newArray.push(flav4[i])
  }
  while (finalArray.length < 31){
    RDN = Math.floor(Math.random() * newArray.length)
    randomFlavors.push(newArray(RDN))
    newArray.splice(RDN,1)
  }
